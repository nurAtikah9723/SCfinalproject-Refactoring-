...
public class Main 
{
    ...
    public static void allFunctionalities(Person person, int choice) throws IOException
    {
        ...
        else if (choice == 2)
        {
            ...
            if (books != null)
            {
                Book b = b(person, lib, input, books);
            }
        }
        ...
        else if (choice == 6)
        {
            ...
            if (books != null)
            {
                Borrower bor = bor(person, lib, input, books);
            }
        }        
	...
	private static Borrower bor(Person person, Library lib, int input, ArrayList<Book> books) {
		input = takeInput(-1, books.size());
	...
	private static Book b(Person person, Library lib, int input, ArrayList<Book> books) {
		input = takeInput(-1, books.size());
		Book b = books.get(input);
